...
export const FileManagerView: React.FC = () => {
  ...
  React.useEffect(() => {
    if (isFileManagerOpen) {
      ...
      drawerTranslation.value = withSpring(width, {
        damping: 20,
        stiffness: 90,
        mass: 0.5,
      });
  ...
  const handleFileOptions = async (file: StoredFile) => {
    Alert.alert("File Options", file.name, [
      {
        text: "Open (OS Native)",
        onPress: async () => {
          try {
      ...
      },
      {
        ...
        onPress: async () => {
          try {
            await Share.share({
              url: file.uri,
              message: `Check out this generated file: ${file.name}`,
            });
          } catch (e) {
            console.error(e);
          }
      ...
      },
      {
        ...
        onPress: () => {
          ...
          Alert.alert(
            "Added to Smart Gen",
            "File appended to current workspace context.",
          );
        },
      },
      {
      ...
      {
        text: starredFiles.includes(file.id)
          ? "Remove Star"
          : "Favorite / Star",
  ...
  const displayFiles = files.filter(
    (f) =>
      ...
      (!focusedModelId || f.modelId === focusedModelId || !f.modelId),
  );
  ...
  return (
    <Animated.View
      style={[styles.drawerContainer, animatedDrawerStyle]}
      pointerEvents={isFileManagerOpen ? "auto" : "none"}
    >
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity
            onPress={() => setFileManagerOpen(false)}
        ...
        <View style={styles.tabsContainer}>
          <TouchableOpacity
            style={[
              styles.tabBtn,
              activeTab === "uploaded" && styles.tabBtnActive,
            ]}
            onPress={() => setActiveTab("uploaded")}
          >
            <Folder
              color={
                activeTab === "uploaded" ? "#fff" : "rgba(255,255,255,0.5)"
              }
              size={16}
            />
            <Text
          ...
          <TouchableOpacity
            style={[
              styles.tabBtn,
              activeTab === "generated" && styles.tabBtnActive,
            ]}
            onPress={() => setActiveTab("generated")}
          >
            <Database
              color={
                activeTab === "generated" ? "#fff" : "rgba(255,255,255,0.5)"
              }
              size={16}
            />
            <Text
              style={[
                styles.tabText,
                activeTab === "generated" && styles.tabTextActive,
              ]}
            >
              Generated
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.tabBtn,
              activeTab === "collection" && styles.tabBtnActive,
            ]}
            onPress={() => setActiveTab("collection")}
          >
            <DownloadCloud
              color={
                activeTab === "collection" ? "#fff" : "rgba(255,255,255,0.5)"
              }
              size={16}
            />
            <Text
          ...
          <View style={styles.previewOverlay}>
            <SafeAreaView style={{ flex: 1 }}>
              ...
              <View style={styles.previewContent}>
                {previewFile.type.includes("image") ? (
                  <View style={styles.previewImageMock}>
                    ...
                    <Text style={styles.previewMockText}>
              ...
              <View style={styles.previewActionRow}>
                ...
                <TouchableOpacity
                  style={styles.previewActionBtnSecondary}
                  onPress={() => {
                    setSourceFile(previewFile);
                    setPreviewFile(null);
                    setFileManagerOpen(false);
                  }}
                >
                  <Text style={styles.previewActionTextSecondary}>
                    Use Source
                  </Text>
                </TouchableOpacity>
              </View>
